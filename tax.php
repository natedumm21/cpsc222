<?php
    ...
    $grossPayPerYear = $grossPay * $weeksInYear;
...
function calculatePercent($estimatedAnnualIncome) {
    if (!is_numeric($estimatedAnnualIncome)) {
        return "Error: Invalid input"; // Handle non-numeric input
    }

    if ($estimatedAnnualIncome <= 0) {
        return "Error: Income must be positive"; // Handle negative or zero income
    }
